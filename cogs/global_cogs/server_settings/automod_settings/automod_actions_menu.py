...
class automod_actions(commands.Cog):
    ...
    async def automod_actions(self, language_file, interaction, message, settings, detection_type, actions_changed=(False, None)):
        ...
        # create embed
        actions_embed = discord.Embed(title=f"{language_file['automod_settings']['dropdown'][detection_type]['label']} {language_file['automod_actions']['title']}", description=f"{language_file['automod_actions']['description']['header']}\n-", color=0x6eb259)
        ...
        if settings[f'automod_{detection_type}_actions'] == []:
            ...
            for action in settings[f'automod_{detection_type}_actions']:
                value = language_file['automod_actions']['dropdown'][action]['desc']
        ...
        #create dropdown
        actions_dropdown = self.actions_dropdown(language_file, detection_type, current_actions)
        ...
        if actions_changed != (False, None):
            ...
            await message.edit(embeds=[inaccuracy_warning_embed, actions_embed, actions_changed_embed], view=actions_dropdown)
        else:
            await message.edit(embeds=[inaccuracy_warning_embed, actions_embed], view=actions_dropdown)
    ...
    class actions_dropdown(discord.ui.View):
        def __init__(self, language_file, detection, current_actions, timeout=120):
            ...
            automod_detections = ['Notify','Cancel','Warn','Hold']
            ...
            self.actions_dropdown.callback = self.actions_dropdownion
        ...
        async def actions_dropdownion(self, interaction: discord.Interaction):
