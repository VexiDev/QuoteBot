...
class Quotes(commands.Cog):
           ...
           # embedVar4.add_field(name="Please react with 1️⃣ , 2️⃣ , 3️⃣ to select which quote you would like to delete or react with ❎ to cancel", inline=False)
            ...
            while True:
                if str(reaction) == '✅':
                    ...
                    await message.delete()
                    log = self.bot.get_cog("Logger")
                    await log.logger(command=f"**Deleted quote** from {user} '{quotee}'", user=ctx.author, channel=ctx.channel, color="#f54242", guild=ctx.message.guild.id)
                    print("Deleted")
    ...
    async def listquotes(self, ctx, user: discord.User, *, page=1):
        ...
        if len(results)>25:
            page1 = discord.Embed (
                title = f'Quotes for {user.name} (Page 1/4)',
            ...
            while loopvar < 24:
                ...
                loopvar = loopvar+1
            page2 = discord.Embed (
                title = f'Quotes for {user.name} (Page 2/4)',
                description = 'A list of all logged quotes for a user',
                colour = 0x00ff00   
            )
            loopvar = 25
